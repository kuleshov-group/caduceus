...
class MLP_VEP_ClinVar(nn.Module):
    """
    MLP head for Variant Effect Prediction (ClinVar).
    ...
    def __init__(self, input_size, hidden_size, output_size):
        super(MLP_VEP_ClinVar, self).__init__()
...
class DNAModelForVEPFinetuning(nn.Module):
    """
    DNA Model for Variant Effect Prediction (ClinVar) fine-tuning.
    ...
    def __init__(self, args):
        ...
        self.head = MLP_VEP_ClinVar(input_size=2*self.inner_dim, hidden_size=2*self.inner_dim, output_size=2)
...
class Lit_ClinVarFinetuning(pl.LightningModule):
    """
    PyTorch Lightning model for fine-tuning on ClinVar Variant Effect Prediction.
    ...
    def on_test_epoch_end(self):
        ...
        self.logger.experiment.log({
            "test/AUROC": val_auroc,
            "test/Accuracy": val_accuracy,
            "test/AUPRC": val_auprc,
        })
...
class VariantEffectPredictionDataModule(pl.LightningDataModule):
    """
    Data module for ClinVar Variant Effect Prediction fine-tuning with PyTorch Lightning.
    ...
    def __init__(self, config):
        ...
        self.bp_per_tokem = config.bp_per_token
...
def finetune(args):
    """
    Main function to start the training process for ClinVar Variant Effect Prediction using PyTorch Lightning.
    ...
    for idx,val_chromosome in enumerate(held_chromosomes):
        ...
        wandb_logger = WandbLogger(
            ...
            project="Variant Effect Prediction ClinVar",
            log_model=True
        )
        data_module._split_dataset(val_chromosome)
        model = Lit_ClinVarFinetuning(args)
